...
if uploaded_file:
    try:
        ...
        if data_col:
            ...
            c1.metric("Tổng số cuộn", n)
            c2.metric("Trung bình", f"{mu:.2f}")
            ...
            c4.metric("Cpk (Khách hàng)", f"{cpk:.2f}" if cpk is not None else "N/A")

            # --- VIEW 1 ---
            if view_mode == "Phân bố & Trending":
                col_left, col_right = st.columns([1, 1.4])
                
                # Tự động tính toán dải trục X để hiển thị đầy đủ các giới hạn
                ...
                with col_left:
                    st.subheader("Trạng thái phân bố (Histogram)")
                    k_bins = math.ceil(1 + 3.322 * math.log10(n)) if n > 0 else 10
                    bin_width = (plot_data.max() - plot_data.min()) / k_bins if n > 1 else 1
                    
                    fig_dist = go.Figure()
                    # Vẽ Histogram số lượng cuộn
                    fig_dist.add_trace(go.Histogram(x=plot_data, nbinsx=k_bins, marker_color='#1E88E5', opacity=0.6, name='Thực tế'))
                    
                    # Đường Normal Curve
                    if sigma > 0:
                        x_c = np.linspace(x_min, x_max, 300)
                        y_c = norm.pdf(x_c, mu, sigma) * n * bin_width
                        fig_dist.add_trace(go.Scatter(x=x_c, y=y_c, mode='lines', line=dict(color='#0D47A1', width=3), name='Chuẩn'))
                    
                    # --- Vẽ các đường giới hạn lên Histogram ---
                    # 1. Giới hạn Khách hàng (Đỏ đậm)
                    if v_lsl_cust: fig_dist.add_vline(x=v_lsl_cust, line_dash="dash", line_color="#D32F2F", line_width=3, annotation_text="Cust Min", annotation_position="top left")
                    if v_usl_cust: fig_dist.add_vline(x=v_usl_cust, line_dash="dash", line_color="#D32F2F", line_width=3, annotation_text="Cust Max", annotation_position="top right")
                    
                    # 2. Giới hạn Nội bộ (Nâu)
                    if v_lsl_int: fig_dist.add_vline(x=v_lsl_int, line_dash="dot", line_color="#5D4037", line_width=2, annotation_text="Int Min")
                    if v_usl_int: fig_dist.add_vline(x=v_usl_int, line_dash="dot", line_color="#5D4037", line_width=2, annotation_text="Int Max")

                    fig_dist.update_layout(template="plotly_white", xaxis_range=[x_min, x_max], yaxis_title="Số lượng cuộn thép", margin=dict(t=30))
                    st.plotly_chart(fig_dist, use_container_width=True)

                with col_right:
                    st.subheader("Biểu đồ Trending & Control")
                    fig_trend = go.Figure()
                    ...
                    # Các đường giới hạn trên trending
                    lines = [
                    ...
                    fig_trend.update_layout(template="plotly_white", margin=dict(r=250), xaxis_title="Thứ tự cuộn", yaxis_title="Giá trị đo")
    ...
    st.info("👈 Vui lòng tải file Excel để phân tích trạng thái phân bố.")
