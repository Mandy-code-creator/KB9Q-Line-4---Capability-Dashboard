    ...
    try:
        ...
        if data_col:
            ...
            if view_mode == "Process Analytics":
                ...
                # GIẢI PHÁP CHỐNG ĐÈ CHỮ: Dành 35% lề trái cho SPC Box, 45% lề phải cho Legend
                x_range = [min_pt - span * 0.35, max_pt + span * 0.45]

                counts, _ = np.histogram(plot_data, bins=k_bins)
                # Tăng chiều cao lên 1.45 để nhãn dọc không chạm vào đỉnh Histogram
                max_y = counts.max() * 1.45 if len(counts) > 0 else 10
                ...
                # THÊM ĐƯỜNG GIỚI HẠN VÀ NHÃN CĂN GIỮA (xanchor="center")
                limits_to_draw = [
                ...
                for val, lbl, clr, sty in limits_to_draw:
                    if val and val not in drawn_values:
                        ...
                        fig_dist.add_annotation(
                            x=val, y=0.96, yref="paper", 
                            text=f"<b>{lbl}: {val}</b>", 
                            textangle=-90, font=dict(color=clr, size=11), 
                            showarrow=False, xanchor="center", yanchor="top",
                            bgcolor="rgba(255, 255, 255, 0.9)", bordercolor=clr, borderwidth=1, borderpad=3
                        )
                ...
                fig_dist.add_annotation(
                    xref="paper", yref="paper", x=0.02, y=0.98, text=box_txt, showarrow=False, align="left",
