...
st.markdown("""
    <style>
    /* Nền Slate nhạt tạo chiều sâu */
    .main { background-color: #f4f7f9; }
    
    /* Thiết kế thẻ Card cho biểu đồ */
    div.stPlotlyChart {
...
def find_data_col(df, key):
    """Tìm cột dữ liệu thực tế (không chứa từ khóa giới hạn)"""
...
def get_valid_limit(df, keyword, limit_type, category):
    """Lấy giá trị giới hạn trung vị, bỏ qua nếu <= 0 hoặc trống"""
...
if uploaded_file:
    try:
        ...
        if data_col:
            ...
            # Tính Cpk (Dựa trên giới hạn khách hàng hoặc nội bộ)
            ...
            # --- VIEW 1: PHÂN BỐ & TRENDING (STACKED) ---
            if "View 1" in view_mode:
                
                # 1. BIỂU ĐỒ PHÂN BỐ (Bố cục 100% chiều ngang)
                ...
                # Vẽ các giới hạn lên Phân bố (Nhãn dọc để không che dữ liệu)
                limit_hist = [
                    (v_lsl_cust, "KHÁCH MIN", "#d32f2f", "dash"),
                    (v_usl_cust, "KHÁCH MAX", "#d32f2f", "dash"),
                    (v_lsl_int, "Nội bộ Min", "#795548", "dot"),
                    (v_usl_int, "Nội bộ Max", "#795548", "dot")
                ]
                for v, lbl, clr, sty in limit_hist:
                    if v:
                        fig_dist.add_vline(x=v, line_dash=sty, line_color=clr, line_width=2)
                ...
                # 2. BIỂU ĐỒ TRENDING (Bố cục 100% chiều ngang)
                ...
                fig_trend.add_trace(go.Scatter(x=plot_data.index, y=plot_data, mode='lines+markers', 
                                              ...
                                              marker=dict(size=6, color='white', line=dict(width=1.5, color='#3498db'))))

                # Danh sách đường giới hạn trên Trending (Offset nhãn để không chồng nhau)
                trend_lines = [
                    (mu, "MEAN", "green", "solid", 1.01),
                    (ucl, "UCL", "#FB8C00", "dash", 1.01),
                    (lcl, "LCL", "#FB8C00", "dash", 1.01),
                    (v_usl_int, "Int Max", "#795548", "dot", 1.09),
                    (v_lsl_int, "Int Min", "#795548", "dot", 1.09),
                    (v_usl_cust, "KHÁCH MAX", "#d32f2f", "dashdot", 1.18),
                    (v_lsl_cust, "KHÁCH MIN", "#d32f2f", "dashdot", 1.18)
                ]
