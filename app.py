...
if uploaded_file:
    try:
        ...
        if "用途碼" in df_raw.columns:
            usage_list = sorted(
                df_raw["用途碼"].dropna().unique().tolist()
            )
        ...
        if data_col:
            ...
            if view_mode == "Process Analytics":
                ...
                # LABELS chỉnh sửa: nằm hẳn bên trái
                def add_dist_vline(val, name, color, dash):
                    if val is not None:
                        fig_dist.add_vline(x=val, line_dash=dash, line_color=color, line_width=5)
                        fig_dist.add_annotation(
                            xref="paper", x=0,  # cố định bên trái
                            y=val,
                            text=f"<b>{name}: {val:.1f}</b>",
                            showarrow=False,
                            xanchor="left",
                ...
                add_dist_vline(v_lsl_tgt, "Cust LSL", "#2E7D32", "solid")
                add_dist_vline(v_usl_tgt, "Cust USL", "#2E7D32", "solid")
                add_dist_vline(v_lsl_std, "Int LSL", "#D32F2F", "dash")
                add_dist_vline(v_usl_std, "Int USL", "#D32F2F", "dash")

                fig_dist.update_layout(template="simple_white", height=650, xaxis_range=x_range, showlegend=False,
                                       font=dict(size=18, family="Arial", color="black"),
                                       margin=dict(t=80, r=80, l=80, b=80))
                fig_dist.update_xaxes(showline=True, linewidth=3, linecolor='black', mirror='all', tickfont=dict(size=18))
                st.plotly_chart(
                    fig_dist,
                ...
                def add_trend_hline(val, name, color, dash, pos):
                    if val is not None:
                        fig_trend.add_hline(
                            ...
                            annotation_position="top left",  # chỉnh label sang trái
                            annotation_font=dict(size=18, color=color, family="Arial Black"),
                            annotation_bgcolor="rgba(255,255,255,0)"
                        )

                add_trend_hline(v_usl_tgt, "Cust USL", "#2E7D32", "solid", "top left")
                add_trend_hline(v_usl_std, "Int USL", "#D32F2F", "dash", "top left")
                add_trend_hline(v_lsl_tgt, "Cust LSL", "#2E7D32", "solid", "top left")
                add_trend_hline(v_lsl_std, "Int LSL", "#D32F2F", "dash", "top left")
                add_trend_hline(ucl, "UCL", "#E67E22", "dot", "top left")
                add_trend_hline(lcl, "LCL", "#E67E22", "dot", "top left")
                ...
                fig_trend.update_xaxes(showline=True, linewidth=3, linecolor='black', mirror='all', tickfont=dict(size=18))
                fig_trend.update_yaxes(showline=True, linewidth=3, linecolor='black', mirror='all', tickfont=dict(size=18))
            ...
            # ==================================
                ...
                fig_imr.add_trace(go.Scatter(
                    y=plot_data, mode='lines+markers',
                    line=dict(color='#1F77B4', width=4), marker=dict(size=9)
                ), row=1, col=1)

                fig_imr.add_trace(go.Scatter(
                    y=mr, mode='lines+markers',
                    line=dict(color='#1F77B4', width=4), marker=dict(size=9)
                ), row=2, col=1)
                ...
                def add_imr_hline(val, label, color, row):
                    if val is not None:
                        fig_imr.add_hline(
                            y=val, line_dash="dash", line_color=color, line_width=5,
                            annotation_text=f"<b>{label}: {val:.1f}</b>",
                            annotation_position="top left",  # chỉnh label sang trái
                            annotation_font=dict(color=color, size=18, family="Arial Black"),
                            annotation_bgcolor="rgba(255,255,255,0)",
                            row=row, col=1
                        )
                ...
                fig_imr.update_layout(
                    height=900, template="simple_white", showlegend=False,
                    font=dict(size=18, family="Arial", color="black"),
                    margin=dict(l=80, r=80, t=80, b=80)
                )
                fig_imr.update_xaxes(showline=True, linewidth=3, linecolor='black', mirror='all', tickfont=dict(size=18))
                fig_imr.update_yaxes(showline=True, linewidth=3, linecolor='black', mirror='all', tickfont=dict(size=18))
