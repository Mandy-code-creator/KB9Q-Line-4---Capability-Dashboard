...
if uploaded_file:
    try:
        ...
        # Cập nhật từ khóa tìm kiếm giới hạn: Dùng chính chữ "YPE"
        if "YS" in short_key:
            zh_key = "降伏強度"
        elif "TS" in short_key:
            zh_key = "抗拉強度"
        elif "EL" in short_key:
            zh_key = "伸長率"
        elif "HRB" in short_key:
            zh_key = "硬度"
        else:
            zh_key = "YPE" # Cột dữ liệu của YPE có tên là YPE
        ...
        if data_col:
            ...
            # Capability Calculations (Internal Standard)
            cp, cpk = None, None
            if sigma > 0:
